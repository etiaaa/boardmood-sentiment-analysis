# -*- coding: utf-8 -*-
"""Figures EDA 'dashboard sombre' sous forme de fonctions reutilisables.

Chaque fonction construit une figure moderne a partir du DataFrame et la retourne
(pour affichage inline dans le notebook via plt.show()). Sauvegarde PNG optionnelle.
"""
import numpy as np
import pandas as pd
...
def fig_hero(df, save=None):
    """Vue d'ensemble : cartes KPI + donut distribution + diverging par compagnie."""
    fig = plt.figure(figsize=(16, 9)); fig.patch.set_facecolor(vz.BG)
    ...
    for i, s in enumerate(ORDRE):
        pct = counts[s]/n*100; x0 = 0.06 + i*0.33
    ...
    # diverging par compagnie
    axb = fig.add_subplot(gs[1:, 2:4])
    ct = (pd.crosstab(df["airline"], df["airline_sentiment"], normalize="index")*100).reindex(columns=ORDRE)
    ct = ct.loc[ct["negative"].sort_values().index]
    for j, (airline, row) in enumerate(ct.iterrows()):
        ...
        axb.text(-neu/2-neg-1.5, j, f"{neg:.0f}%", ha="right", va="center", color=vz.NEG, fontsize=10, fontweight="bold")
        axb.text(neu/2+pos+1.5, j, f"{pos:.0f}%", ha="left", va="center", color=vz.POS, fontsize=10, fontweight="bold")
    axb.axvline(0, color=vz.MUTED, lw=1, ls=(0,(3,3)), alpha=0.6, zorder=1)
    axb.set_yticks(range(len(ct))); axb.set_yticklabels(ct.index, color=vz.TEXT, fontsize=11)
    axb.set_xticks([]); axb.set_xlim(-95, 60); axb.grid(False)
    axb.set_title("Sentiment par compagnie", fontfamily=vz.TITLE_FONT, fontsize=16, color=vz.TEXT, loc="left", pad=28)
    axb.text(0.0, 1.06, "◄ négatif", transform=axb.transAxes, color=vz.NEG, fontsize=11, fontfamily=vz.BODY_FONT, fontweight="bold", ha="left", va="bottom")
    axb.text(0.5, 1.06, "neutre", transform=axb.transAxes, color=vz.NEU, fontsize=11, fontfamily=vz.BODY_FONT, ha="center", va="bottom")
    axb.text(1.0, 1.06, "positif ►", transform=axb.transAxes, color=vz.POS, fontsize=11, fontfamily=vz.BODY_FONT, fontweight="bold", ha="right", va="bottom")
    _save(fig, save); return fig


def fig_motifs(df, save=None):
    """Lollipop degrade + glow des motifs d'insatisfaction."""
    reasons = (df.loc[df.airline_sentiment=="negative","negativereason"].value_counts().head(10).sort_values())
    fig, ax = plt.subplots(figsize=(12, 7))
    ys = np.arange(len(reasons)); maxv = reasons.max()
    for y, (lab, v) in zip(ys, reasons.items()):
        frac = v/maxv; color = plt.get_cmap(vz.CMAP_NEG)(0.35 + 0.65*frac)
        ...
        ax.text(v+maxv*0.02, y, f"{v:,}".replace(",", " "), va="center", ha="left", color=vz.TEXT, fontsize=11, fontweight="bold")
    ax.set_yticks(ys)
    ax.set_yticklabels([l.replace(" ", "\n") if len(l) > 16 else l for l in reasons.index], color=vz.TEXT, fontsize=10)
...
def fig_ridgeline(df, save=None):
    """Ridgeline de la longueur des tweets (n mots) par sentiment."""
    if "n_mots" not in df: df = df.assign(n_mots=df["text"].str.split().str.len())
    fig, ax = plt.subplots(figsize=(12, 6.5))
    xs = np.linspace(0, 38, 400); step = 0.9
    for i, s in enumerate(ORDRE):
        data = df.loc[df.airline_sentiment==s, "n_mots"].dropna()
        kde = gaussian_kde(data); dens = kde(xs); dens = dens/dens.max()
        base = (len(ORDRE)-1-i)*step; c = vz.SENT_COLORS[s]
        ...
        ax.text(0.5, base+0.08, s.upper(), color=c, fontsize=12, fontweight="bold", fontfamily=vz.TITLE_FONT)
        ax.text(34, base+0.55, f"médiane\n{data.median():.0f} mots", color=vz.MUTED, fontsize=9, ha="center", va="center")
...
def fig_wordclouds(df, save=None):
    """Nuages de mots par sentiment sur fond sombre (sur text_clean)."""
    if "text_clean" not in df:
        raise ValueError("Le DataFrame doit contenir 'text_clean'")
    cmaps = {"negative": vz.CMAP_NEG,
             "neutral": LinearSegmentedColormap.from_list("n", ["#5C6B82", vz.NEU]),
             "positive": vz.CMAP_POS}
    fig, axes = plt.subplots(1, 3, figsize=(17, 5.6)); fig.patch.set_facecolor(vz.BG)
    for ax, s in zip(axes, ORDRE):
        ...
        ax.set_title(s.upper(), color=vz.SENT_COLORS[s], fontfamily=vz.TITLE_FONT, fontsize=18, fontweight="bold", pad=10)
...
def fig_confiance(df, save=None):
    """Histogramme degrade + KDE lumineuse de la confiance d'annotation."""
    fig, ax = plt.subplots(figsize=(12, 6))
    vals = df["airline_sentiment_confidence"].dropna()
    counts_h, edges = np.histogram(vals, bins=28, range=(0.3, 1.0))
    centers = (edges[:-1]+edges[1:])/2; cmap = plt.get_cmap(vz.CMAP_CYAN)
    ...
    ax.text(0.32, counts_h.max()*0.9, f"{(vals<0.6).mean()*100:.1f}%\nsous 0.6", color=vz.MUTED, fontsize=11, va="top")
